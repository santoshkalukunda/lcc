...
class SearchController extends Controller
{
    ...
    function searchlist(Request $request)
    {
        ...
        $count=CompanyInfo::where('name','like',"$data%")->orWhere('address','like',"$data%")->orWhere('contact_no','like',"$data%")->orWhere('reg_no','like',"$data%")->orWhere('fiscal_year','like',"$data%")->orWhere('reg_date','like',"$data%")->orWhere('category','like',"$data%")->count();
        ...
        if($request->operation!=null and $request->days!=null){
            ...
            $count =Namechange::where('new_name', 'like', "$name%")->Where('status', 'like', "$status%")->where('change_date',"$request->operation","$ago")->count();
        ...
        $count =Namechange::where('new_name', 'like', "$name%")->Where('status', 'like', "$status%")->count();
         return view('report.namechange')->with('namechange',$search)->with('count',$count)->with('currentdate',$currentdate);
        }
        ...
        public function documentreport(Request $request){
          $currentdate=nepalicurrenntdate();
           ...
           $documentreport=DB::table('documentreports')->join('company_infos','documentreports.company_id','=','company_infos.id')->select('documentreports.*','company_infos.*')->Where('documentreports.status', 'like', "$status%")->where('company_infos.name','like',"$request->name%")->where('company_infos.reg_date',$request->operation,"$date")->orderBy('company_infos.reg_date')->paginate(10);    
           $count=DB::table('documentreports')->join('company_infos','documentreports.company_id','=','company_infos.id')->select('documentreports.*','company_infos.*')->Where('documentreports.status', 'like', "$status%")->where('company_infos.name','like',"$request->name%")->where('company_infos.reg_date',"$request->operation","$date")->count();    
            ...
            $count=DB::table('documentreports')->join('company_infos','documentreports.company_id','=','company_infos.id')->select('documentreports.*','company_infos.name','company_infos.contact_no','company_infos.reg_date')->Where('documentreports.status', 'like', "$status%")->where('company_infos.name','like',"$request->name%")->orderBy('company_infos.reg_date')->count();
            return view('report.document')->with('documentreport',$documentreport)->with('current_date',$currentdate)->with('count',$count);;
           }
           }
           public function audit(Request $request){
            ...
            if($request->status=="audited"){
                ...
                $count = Auditreport::Join('company_infos','auditreports.company_id','=','company_infos.id')->select('company_infos.name','company_infos.contact_no','auditreports.*')->where('auditreport_fiscal', '=', "$data->fiscal")->where('auditreport_reg_fiscal', '!=', "$data->fiscal")->where('company_infos.name','like',"$request->name%")->count();
            ...
            if($request->status=="not_audited"){
                ...
                $count= Auditreport::Join('company_infos','auditreports.company_id','=','company_infos.id')->select('company_infos.name','company_infos.contact_no','auditreports.*')->where('auditreport_reg_fiscal', '!=', "$data->fiscal")->where('company_infos.name','like',"$request->name%")->count();
           ...
           public function renew(Request $request){
            ...
            if($request->status=="renewed"){
                ...
                $count = Renewreport::Join('company_infos','renewreports.company_id','=','company_infos.id')->select('company_infos.name','company_infos.contact_no','renewreports.*')->where('renewreport_fiscal', '=', "$data->fiscal")->where('renewreport_reg_fiscal', '!=', "$data->fiscal")->where('company_infos.name','like',"$request->name%")->count();
            ...
            if($request->status=="not_renew"){
              ...
              //$search =CompanyInfo::with(['renew'])->where('fiscal_year','!=',"$data->fiscal")->where('name','like',"$request->name%")->paginate(10);
                ...
                $count = Renewreport::Join('company_infos','renewreports.company_id','=','company_infos.id')->select('company_infos.name','company_infos.contact_no','renewreports.*')->where('renewreport_reg_fiscal', '!=', "$data->fiscal")->where('company_infos.name','like',"$request->name%")->count();
                 //$search =CompanyInfo::with(['renew'])->where('fiscal_year','!=',"$data->fiscal")->where('name','like',"$request->name%")->paginate(10);
                //$search=Renew::paginate(10);
                return view('report.renew')->with('renew',$search)->with('date',$data)->with('count',$count);
