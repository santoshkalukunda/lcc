...
(function ($) {
  var $body = $('body'); // initialize date converter

  var converter = new DateConverter();
  converter.setCurrentDate();
  var today = converter.nepaliYear + '-' + converter.nepaliMonth + '-' + converter.nepaliDate; // disable autocomplete

  $('form').attr('autocomplete', 'off'); // give each calendar instance a unique id

  var cal_no = 1;
  var cal_id = ''; // set default date to this month

  var this_year = converter.getNepaliYear();
  var this_month = converter.getNepaliMonth(); // calendar config

  var start_year = 2000;
  var end_year = 2098; // is this single datepicker

  var single_datepicker = 0; // calendar ui selector

  var $year_select = '';
  var $month_select = '';
  var $days_container = ''; // for multiple selection
  ...
  var input_field_name = ''; // show different message according to os
  ...
  $.fn.nepaliDatePicker = function (options) {
    ...
    $(this).each(function () {
      ...
      cal_no++; // add common class to all input fields

      $(this).addClass('andp-date-picker'); // this will run only once
      ...
      if (default_value && !single_datepicker) {
        // set form 
        $form = $(this).parents('form'); // set calendar id

        cal_id = $(this).data('cal_id'); // this will be used to generate hidden input fields with same input name
    ...
    $(this).click(function () {
      ...
      cal_id = $(this).data('cal_id'); // initiate calendar ui

      init(this);

      if (single_datepicker) {
        // inline calendar
        selected_date = $(this).val(); // add this date into selected dates arary
        // switch calendar to selected month and year

        if (selected_date.length > 0) {
          older_date_ar = selected_date.split('-');
          $month_select.val(older_date_ar[1]).change();
          $year_select.val(older_date_ar[0]).change(); // select default date
        ...
        $form = $(this).parents('form'); // this will be used to generate hidden input fields with same input name
        ...
        if (total_hidden_dates > 0) {
          ...
          if (total_hidden_dates == 1) {
            ...
            // last selected date
            older_date = $('input.andp-hidden-dates[data-cal_id="' + cal_id + '"]:last-child').val(); // switch calendar to last month and year of selected date
  ...
  $body.on('click', '.andp-datepicker-container.open .andp-days-numbers .day', function (event) {
    ...
    var $sel_calendar = $('.andp-datepicker-container[data-cal_id="' + cal_id + '"]'); // disable shift or ctrl key on single_datepicker

    if (single_datepicker) {
      // reset user_selected_dates
      user_selected_dates = []; // remove all other selected class

      $sel_calendar.find('.andp-column .day').removeClass('selected'); // select date function
      ...
      if (event.shiftKey) {
        ...
        if (total_captured_dates > 0) {
          selected_date = $(this).data('date');
          last_captured_date = user_selected_dates[total_captured_dates - 1]; // get older date
          ...
          var days_difference = get_days_difference(selected_date, last_captured_date); // reset all captured dates
        ...
        // reset user_selected_dates
        user_selected_dates = []; // remove other selected class

        $sel_calendar.find('.andp-column .day').removeClass('selected'); // select correct day

        select_date(selected_date); // show message for multiple selection
  ...
  $body.on('click', function (e) {
    var container = $(".andp-datepicker-container, input.andp-date-picker"); // if the target of the click isn't the container nor a descendant of the container
  ...
  function init(this_sel) {
    ...
    $('.andp-datepicker-container').removeClass('open').hide(); // check if calendar ui has already been generated for selected cal_id
    ...
    template += '</div>';
    template += '</div>';
    template += '</div>'; // insert into DOM

    $body.append(template); // re-initiate var, wont work otherwise

    $sel_calendar = $('.andp-datepicker-container[data-cal_id="' + cal_id + '"]');
    $year_select = $sel_calendar.find('.andp-year-select');
    $month_select = $sel_calendar.find('.andp-month-select');
    $days_container = $sel_calendar.find('.andp-days-numbers'); // add month into month select
    ...
    $month_select.append(append_html); // add year into year select
  ...
  function select_date(selected_date) {
    var soft_select = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    ...
    if (soft_select) {
      ...
      if (ar_index < 0) {
        ...
        user_selected_dates.push(selected_date); // mark this day as selected

        $this.addClass('selected');
      } else {
        // date already added
        // remove this date from array
        user_selected_dates.splice(ar_index, 1); // mark as not selected
  ...
  function DateConverter() {
    ...
    [31, 31, 31, 32, 31, 31, 29, 30, 30, 29, 30, 30], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 32, 31, 30, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], [31, 31, 31, 32, 31, 31, 29, 30, 30, 29, 30, 30], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 30, 29, 31], [31, 31, 31, 32, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 30, 29, 31], [31, 31, 31, 32, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], [30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 29, 31], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 31, 32, 30, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], [30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 29, 31], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 32, 31, 30, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], [30, 32, 31, 32, 31, 31, 29, 30, 30, 29, 29, 31], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 32, 31, 30, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], [31, 31, 31, 32, 31, 31, 29, 30, 30, 29, 30, 30], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 32, 31, 30, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], [31, 31, 31, 32, 31, 31, 29, 30, 30, 29, 30, 30], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], [31, 31, 31, 32, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 30, 29, 31], [31, 31, 31, 32, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 30, 29, 31], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 31, 32, 30, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], [30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 29, 31], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 32, 31, 30, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], [30, 32, 31, 32, 31, 31, 29, 30, 29, 30, 29, 31], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 32, 31, 30, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], [31, 31, 31, 32, 31, 31, 29, 30, 30, 29, 29, 31], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 32, 31, 30, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], [31, 31, 31, 32, 31, 31, 29, 30, 30, 29, 30, 30], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], //2071
    [31, 32, 31, 32, 31, 30, 30, 29, 30, 29, 30, 30], //2072
    [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], //2073
    [31, 31, 31, 32, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 30], // 2076
    [31, 32, 31, 32, 31, 30, 30, 30, 29, 30, 29, 31], [31, 31, 31, 32, 31, 31, 30, 29, 30, 29, 30, 30], [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30], [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 30], [31, 31, 32, 32, 31, 30, 30, 30, 29, 30, 30, 30], [30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 30, 30], [31, 31, 32, 31, 31, 30, 30, 30, 29, 30, 30, 30], [31, 31, 32, 31, 31, 30, 30, 30, 29, 30, 30, 30], [31, 32, 31, 32, 30, 31, 30, 30, 29, 30, 30, 30], [30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 30, 30], [31, 31, 32, 31, 31, 31, 30, 30, 29, 30, 30, 30], [30, 31, 32, 32, 30, 31, 30, 30, 29, 30, 30, 30], [30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 30, 30], [30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 30, 30], //2090
    ...
    this.setEnglishDate = function (year, month, date) {
      ...
      this.englishDate = date; //Setting nepali reference to 2000/1/1 with english date 1943/4/14

      this.nepaliYear = 2000;
      this.nepaliMonth = 1;
      this.nepaliDate = 1;
      var difference = this.getEnglishDateDifference(1943, 4, 14); //Getting nepali year untill the difference remains less than 365
    ...
    this.setNepaliDate = function (year, month, date) {
      if (!this.isNepaliRange(year, month, date)) {
        console.log('Invalid Date Format'); // throw new Exception("Invalid date format.");
      ...
      this.nepaliDate = date; //Setting english reference to 1944/1/1 with nepali date 2000/9/17

      this.englishYear = 1944;
      this.englishMonth = 1;
      this.englishDate = 1;
      var difference = this.getNepaliDateDifference(2000, 9, 17); //Getting english year untill the difference remains less than 365
